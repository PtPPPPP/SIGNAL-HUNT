...
- 完整备份与恢复
- 持久化诊断日志（IndexedDB，可导出 JSON，自动脱敏）
...
> 概率控制台（`/admin/pacing`）的 UI 流程不在 E2E 中驱动。它当前是另一个 Agent 的重构区域；概率持久化与「奖池使用配置」由 E2E-4 / E2E-5 以确定性方式覆盖。

## 数据规模与性能

`drawRecords` 在 schema v3 增加了 `[eventId+prizeId]` 与 `[eventId+status]` 复合索引。提交抽奖（`commitPersistentDraw`）不再读取全表记录，而是按当前活动索引读取，节奏（Pacing）计算也只扫描当前活动的记录。
...
诊断日志持久化在 IndexedDB 的 `diagnosticLogs` 表（schema v3），刷新或崩溃后仍然保留：
...
日志在写入前会做隐私脱敏：手机号、参与者 PII、邮箱等敏感字段会被遮蔽或 `[redacted]`，且日志不会随备份导出，也不会上传云端。
...
1. 拆分大型后台页面：`AdminPacingPage`、`AdminDiagnosticsPage`、`AdminPrizesPage` 已经偏大，后续维护成本会升高（部分页面正在由其它 Agent 重构）。
...
- Quantum Design Logo 使用 Vite `BASE_URL` 解析，浏览器和 packaged Electron 都从本地 `brand/quantum-design-logo.png` 加载
- 使用单实例启动，重复打开会聚焦已有窗口
- 主窗口关闭外链跳转和新窗口
